...
force = np.empty((3, N), dtype=Variable)
...
for i in range(N):
    ...
    force[:, i] = prog.NewContinuousVariables(3, 'force' + str(i))
x[:, N] = prog.NewContinuousVariables(n, 'x' + str(N))


f_max = 10.0  # Max magnitude for each component of the force
for i in range(N):
    prog.AddBoundingBoxConstraint(
        -f_max * np.ones(3), f_max * np.ones(3), force[:, i])
...
def eval_vel_constraints(z):
    ...
    v_curr = z[nq:n]
    v_next = z[n+nq:2*n]
    applied_force = z[2*n:2*n+3]

    v_next[3] += (applied_force[0] / mass) * dt
    v_next[4] += (applied_force[1] / mass) * dt
    v_next[5] += (applied_force[2] / mass) * dt
...
for i in range(N+1):
    ...
    if i < N:
        v_vars = np.hstack((x[:, i], x[:, i+1], force[:, i]))
...
force_sol = result.GetSolution(force)

# Plot the Force Applied over Time Steps
plt.figure(figsize=(10,6))

plt.subplot(3,1,1)
plt.step(range(N), force_sol[0, :], where='post')
plt.title('Force in X direction over time steps')
plt.ylabel('Force (N)')

plt.subplot(3,1,2)
plt.step(range(N), force_sol[1, :], where='post')
plt.title('Force in Y direction over time steps')
plt.ylabel('Force (N)')

plt.subplot(3,1,3)
plt.step(range(N), force_sol[2, :], where='post')
plt.title('Force in Z direction over time steps')
plt.ylabel('Force (N)')
